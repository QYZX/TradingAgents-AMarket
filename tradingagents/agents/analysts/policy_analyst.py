    ...
    def policy_analyst_node(state):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                log_llm_call("policy_analyst", f"messages={input_messages}")
                result = chain.invoke(input_messages)
                log_llm_call("policy_analyst", f"messages={input_messages}", f"result={result}")
                break
            except Exception as e:
                last_exception = e
                log_llm_call("policy_analyst", f"messages={state['messages']}", error=e)
