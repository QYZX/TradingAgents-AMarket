...
5. 关键支撑位和阻力位"""
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                log_llm_call("market_analyst", f"messages={input_messages}")
                result = chain.invoke(input_messages)
                log_llm_call("market_analyst", f"messages={input_messages}", f"result={result}")
                break
            except Exception as e:
                last_exception = e
                log_llm_call("market_analyst", f"messages={state['messages']}", error=e)
